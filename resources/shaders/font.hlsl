...
VS_OUTPUT VS(VS_INPUT input, uint vertexID : SV_VertexID)
{
    ...
    // Set texture coordinate based on which vertex it is (uv)
    output.texCoord = foat2(input.texCoord.x + (input.texCoord.z * uv.x), input.texCoord.y + (input.texCoord.w * uv.y));
